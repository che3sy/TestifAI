...
function fetchData() {
  ...
  const altEndpoint = urlParams.get("altEndpoint") === "true";

  let apiUrl;
  if (altEndpoint) {
    apiUrl = `https://x4f6ezfwiloumemi2tkgozxltu0ufwjn.lambda-url.us-west-2.on.aws/make_AP_quiz?subject=${subject}&questionNum=${amount}&difficulty=${difficulty}`;
  } else {
    apiUrl = `https://x4f6ezfwiloumemi2tkgozxltu0ufwjn.lambda-url.us-west-2.on.aws/make_quiz?subject=${subject}&questionNum=${amount}&difficulty=${difficulty}`;
  }
...
function displayQuestions(questions) {
  ...
  questions.forEach((question) => {
    ...
    questionTitle.innerHTML = question.question; // Use innerHTML to allow LaTeX notation
    card.appendChild(questionTitle);

    const optionsList = document.createElement("div");
    question.options.forEach((option) => {
      const optionButton = document.createElement("button");
      optionButton.innerHTML = option; // Use innerHTML to allow LaTeX notation
      optionButton.className = "option-button";
      optionButton.addEventListener("click", () =>
        handleOptionClick(optionButton, question.answer),
      );
